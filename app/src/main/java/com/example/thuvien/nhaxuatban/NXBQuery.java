...
public class NXBQuery {
    ...
    public List<NXB> timKiemNXB(String keyword) {
        List<NXB> list = new ArrayList<>();
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        String key = "%" + keyword + "%";

        Cursor cursor = db.rawQuery(
                "SELECT MaNXB, TenNXB, DiaChi, Email, Sdt FROM nhaxuatban WHERE MaNXB LIKE ? OR TenNXB LIKE ? OR Email LIKE ? ORDER BY MaNXB ASC",
